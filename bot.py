...
class EngagementBot(commands.Bot):
    ...
    async def on_ready(self):
        ...
        # Fallback si setup_hook ne s'est pas exécuté
        if not self._startup_done or not self.cogs:
            logger.warning("No cogs loaded yet in on_ready, loading them now...")
            try:
                await self.setup_hook()
            except Exception as e:
                logger.critical(f"Failed to load cogs in on_ready: {e}", exc_info=True)
                return

        logger.info(f"Cogs loaded: {list(self.cogs.keys())}")
        logger.info(f"Registered commands count: {len(self.commands)}")
        logger.info(f"Registered commands: {[c.name for c in self.commands]}")

        # Éviter de refaire certaines actions si Discord reconnecte
        if self._ready_once:
            return
        ...
        try:
            ...
            if before.channel is None and after.channel is not None:
                ...
                if session:
                    ...
                    if time_spent >= 300:
                        points = min(int(time_spent / 60) * 2, 120)
